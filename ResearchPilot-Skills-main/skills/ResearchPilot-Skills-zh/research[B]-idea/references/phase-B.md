...
> **文献先行**：本阶段每一次生成或调整 idea（技术框架 / pipeline / Method）之前，都必须遵守上文"文献阅读原则"——必须精读 `docs/papers/` 中与当前设计决策直接相关的论文（≥3 篇），理解其核心方法和借鉴点；现有文献不足以支撑设计决策时再重新走下载流程。这一要求对初次深化和后续（含从阶段 D/E 回溯）的每一次调整都成立。
...
> 目标：把框架细化为可执行的完整流程。**输出必须是大白话，用"第一步…第二步…"形式，每步说明输入是什么、做什么处理、输出是什么；禁止出现公式、张量维度描述或伪代码。**
